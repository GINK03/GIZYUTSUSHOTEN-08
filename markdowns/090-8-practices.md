## 8. 練習
この章では、いくつかの目的を設定してそれぞれのユースケース別にどうスクレイピングをしていくのか実演形式でお伝えします。
...
このサイトは画像を大量に公開しているサイトで、商用利用等を含めて競合しない限り自由に利用してOKというとても太っ腹なサイトです。  
...
様々な角度でこのウェブサイトの構造を検証したところ、以下のことが判明しました。
 ...
 - divのclass名にページごとのユニークなhashが与えられており、指定することができない
...
異常検知に引っかかると、IPごとBANになるためこれを避けながらダウンロードする術があるので、ご紹介したいと思います。  
...
<div align="center">
 ...
 `spys.one/en` を更に別の章で紹介したgoogle chrome + seleniumでのダウンロードを用いることにより、Proxyの取得すら自動化できるので、ほぼ制限なしに使えます。
